...
public class FacturaService {
    ...
    public Factura save(Factura factura) {
        ...
        if (isNew) {
            try {
                notifClient.facturaEmisa(NotifEvent.builder()
                        .tip("FACTURA_EMISA")
                        .referintaId(saved.getId())
                        .mesaj("Factura emisa: " + saved.getNumarFactura())
                        .build());
            } catch (Exception e) {
                log.warn("Notificare factura-emisa esuata: {}", e.getMessage());
            }
        }
        return saved;
    }

    public Factura updateStatusPlata(Long id, Factura.StatusPlata newStatus) {
        Factura f = facturaRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Factura", id));
        log.info("Updating invoice {} status: {} -> {}", id, f.getStatusPlata(), newStatus);
        f.setStatusPlata(newStatus);
