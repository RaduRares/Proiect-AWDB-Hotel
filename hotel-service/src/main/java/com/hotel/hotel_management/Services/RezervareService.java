...
import com.hotel.hotel_management.Client.NotifClient;
import com.hotel.hotel_management.Client.NotifEvent;
...
public class RezervareService {
    ...
    public Rezervare save(Rezervare rezervare) {
        ...
        boolean isNew = rezervare.getId() == null;
        Rezervare saved = rezervareRepository.save(rezervare);
        log.info("Saving reservation id: {}", saved.getId());
        if (isNew) {
            try {
                notifClient.rezervareCreata(NotifEvent.builder()
                        .tip("REZERVARE_CREATA")
                        .referintaId(saved.getId())
                        .mesaj("Rezervare noua: id=" + saved.getId())
                        .build());
            } catch (Exception e) {
                log.warn("Notificare rezervare-creata esuata: {}", e.getMessage());
            }
        }
        return saved;
    }

    public Rezervare updateStatus(Long id, Rezervare.StatusRezervare newStatus) {
        Rezervare rez = rezervareRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Rezervare", id));
        log.info("Updating reservation {} status: {} -> {}", id, rez.getStatus(), newStatus);
        rez.setStatus(newStatus);
        return rezervareRepository.save(rez);
    }
    ...
    public void deleteById(Long id) {
        ...
        try {
            notifClient.rezervareAnulata(NotifEvent.builder()
                    .tip("REZERVARE_ANULATA")
                    .referintaId(id)
                    .mesaj("Rezervare stearsa: id=" + id)
                    .build());
        } catch (Exception e) {
            log.warn("Notificare rezervare-anulata esuata: {}", e.getMessage());
        }
    }
    ...
    private void validateDisponibilitate(Rezervare rezervare) {
        ...
        if (numarTotal == 0) {
            throw new BusinessValidationException(
                    "Camera este temporar indisponibila (marcata ca inactiva de administrator).");
        }
